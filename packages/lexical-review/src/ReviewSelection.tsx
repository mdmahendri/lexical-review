...
function nodeDeleteByReviewType(
  ...
  if (node.hasReviewType("addition")) {
    // delete only part of text in <ins> tag
    node.deleteAdditionText(offset, del);
    ...
    // share some similarity to addition, but instead of
...
function suggestDeletion(selection: RangeSelection) {
  ...
  selectedNodes.forEach((node) => {
    if (
      ...
      if (node.hasReviewType("original")) {
        ...
        // addition in the middle, just remove it
        node.remove();
      }
  ...
  if ($isReviewTextNode(firstNode) && $isReviewTextNode(lastNode)) {
    ...
    if (firstNode !== lastNode) {
      ...
      // selection is done in word level for non addition, this checks prevent
...
export function $markForDelete(
  ...
  if (selection.isCollapsed()) {
    ...
    //different behavior only needed for addition, to accomodate a typing error
    if (
      $isReviewTextNode(anchorNode) &&
      anchorNode.hasReviewType("addition") &&
      isBackward
    ) {
...
function insertReviewTextNode(
  ...
  let newReviewTextNode = $createReviewTextNode(text, "addition");
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if ($isRangeSelection(currentSelection)) {
        ...
        if (part === "\n" || part === "\r\n") {
          ...
          const newReviewTextNode = $createReviewTextNode(part, "addition");
...
export function $markTypingInsert(
  ...
  if (
    ...
    if ($isParagraphNode(anchorNode)) {
      const newReviewTextNode = $createReviewTextNode(text, "addition");
  ...
  // selection of review type addition or original
